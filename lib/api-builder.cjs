...
const Router = require('@koa/router') // throwing undefined on node 14 runners on github
...
for (const method of methods) {
  ApiBuilder.prototype[method] = function (uri, ...middlewares) {
    if (!(this instanceof ApiBuilder)) { throw new Error("don't destructure the builder") }
...
ApiBuilder.prototype.build = function build () {
  ...
  // rewind to the root element if not already there
